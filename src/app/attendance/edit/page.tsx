...
const EditAttendanceContent: React.FC = () => {
    ...
    const handleClickStudent = (studentId: number) => {
        ...
        if (student) {
            ...
            setStudentStatus((prev) => ({
                ...prev,
                [studentId]: prev[studentId] || student.status
            }));
    ...
    if (!grade && !section) router.push("/attendance/edit/portal");

    return (
        <>
            {isLargeScreen ? <CoreNormalAppbar navdrawerOpener={toggleNavDrawer} title="Edit Attendance" /> : <CoreSearchbarAppbar title="Edit Attendance" />}
                    ...
                    <List heading={`Students of Grade ${grade} - Section ${section} ${stream ? stream : ""}`}>
                        {displayedData.map((student) => (
                            <ListItem 
                                ...
                                supportingText={`${studentStatus[student.id] || student.status} - ${student.grade}`} 
                                info={student.uid}
